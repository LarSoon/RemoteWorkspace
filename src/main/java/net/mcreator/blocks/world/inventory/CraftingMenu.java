...
public class CraftingMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public CraftingMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		super(QzModMenus.CRAFTING, id);
		...
		for (int si = 0; si < 3; ++si)
			for (int sj = 0; sj < 9; ++sj)
				this.addSlot(new Slot(inv, sj + (si + 1) * 9, 34 + 8 + sj * 18, 16 + 84 + si * 18));
		for (int si = 0; si < 9; ++si)
			this.addSlot(new Slot(inv, si, 34 + 8 + si * 18, 16 + 142));
	}
	...
	public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
		...
		if (event.phase == TickEvent.Phase.END && entity.containerMenu instanceof CraftingMenu) {
